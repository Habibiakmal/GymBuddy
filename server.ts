...
function generateWelcomeMessages(userData: ReturnType<typeof calculateUserData>): string[] {
  ...
  // 1. Personalized Greeting based on Name + Gender + Age Group + Persona
  let greeting = "";
  if (isMax) {
    if (isLansia) {
      const honorific = isFemale ? "Bu" : "Pak";
      greeting = `Halo ${honorific} ${cleanName}! Setup GymBuddy Anda sudah selesai. 💪 Saya Coach Max, siap mendampingi target kebugaran Anda dengan panduan yang aman, terukur, dan terarah.`;
    } else if (isAnak) {
      greeting = `Halo ${cleanName}! Setup GymBuddy kamu sudah beres! 🌟 Aku Coach Max, siap bantu kamu makin sehat, aktif, dan bugar!`;
    } else if (isFemale) {
      greeting = `Alright ${cleanName}! Setup GymBuddy kamu sudah selesai. 💪 Gue Coach Max, siap bantu dan kawal progress kebugaran kamu to-the-point!`;
    } else {
      greeting = `Yo ${cleanName}! Setup GymBuddy lo udah beres. 💪 Gue Coach Max, siap bantu lo push limit dan capai target kebugaran!`;
    }
  } else {
    // Coach Mia
    if (isLansia) {
  ...
  // 2. Summary of Onboarding Data (Clean, verified data only - NO fake history)
  ...
  let summarySection = `📋 *SUMMARY HASIL ONBOARDING:*\n` +
    `• *Nama*: ${cleanName}\n` +
    `• *Gender*: ${genderLabel}\n` +
  ...
  // 3. Explanation of Personalization
  ...
  // 4. Nutrition AI Guide
  let nutritionGuide = "";
  if (showNutrition) {
    if (isMax) {
      const pronoun = isFemale || isLansia ? "kamu" : "lo";
      nutritionGuide = `🥦 *Nutrition AI*\n` +
  ...
  // 5. Workout Coach Guide & Spontaneous Activities Note
  let workoutGuide = "";
  if (showWorkout) {
    if (isMax) {
      const pronoun = isFemale || isLansia ? "kamu" : "lo";
      workoutGuide = `🏋️‍♂️ *AI Workout Coach*\n` +
      ...
      workoutGuide = `🧘‍♀️ *AI Workout Coach*\n` +
  ...
  if (isMax) {
    if (isLansia) {
      cta = `Kalau sudah siap, Anda bisa mulai dengan mengirimkan menu makanan pertama atau menanyakan panduan latihan. 💪`;
    } else if (isFemale) {
      cta = `Udah siap? Kirim makanan pertama kamu atau tanya saya soal workout sekarang! 🔥`;
    } else {
      cta = `Udah siap? Kirim makanan pertama lo atau tanya gue soal workout sekarang! 🔥`;
    }
  } else {
    if (isLansia) {
...
function handleWorkoutProgressLogging(
  ...
  // Try matching individual exercise first
  let matchedEx: any = null;
  for (const ex of existingExercises) {
    const exNameLower = String(ex.name || "").toLowerCase().replace(/[^a-z0-9\s]/g, " ");
    const exWords = exNameLower.split(/\s+/).filter((w: string) => w.length > 2);
  ...
  // Also try alias matching from EXERCISE_DATABASE
  if (!matchedEx) {
    const dbEx = findExerciseOrEquipment(userText);
    if (dbEx) {
      for (const ex of existingExercises) {
        if (ex.name.toLowerCase().includes(dbEx.name.toLowerCase()) ||
            dbEx.name.toLowerCase().includes(ex.name.toLowerCase()) ||
            (dbEx.aliases && dbEx.aliases.some((a: string) => ex.name.toLowerCase().includes(a.toLowerCase())))) {
          matchedEx = ex;
          break;
        }
      }
  ...
  if (matchedEx) {
    ...
    matchedEx.setsState = (matchedEx.setsState || []).map((_: boolean, idx: number) => idx < count);
    ...
    const nextEx = existingExercises.find((e: any) => e.id !== matchedEx.id && e.status !== "completed");
    ...
    if (count >= matchedEx.targetSets) {
      if (nextEx) {
        ...
        return [
          `Nice! *${matchedEx.name}* sudah ${count}/${matchedEx.targetSets} set selesai. ✅\n\n` +
          `Semua latihan hari ini sudah selesai! Luar biasa! 🔥💪`
        ];
      }
    } else {
...
1. JANGAN MENGARANG IDENTITAS/KONTEKS (NO HALLUCINATED ENTITIES):
   - Jika pengguna menyebut nama orang, tokoh, produk, karakter, atau entitas yang tidak memiliki konteks yang cukup (contoh: "Aku mau jadi si A", "Menurut kamu aku bisa kayak A nggak?", "Aku pengen punya badan kayak X", "Targetku seperti A", "Makananku kayak si B"):
     ...
     • TANYAKAN KLARIFIKASI SECARA SINGKAT & ALAMI:
       Contoh Coach Max: "Si A yang lo maksud siapa nih bro? Kasih tahu namanya atau sedikit konteks tentang dia biar gue nggak salah nangkep. 💪"
       Contoh Coach Mia: "Si A yang kamu maksud siapa ya? 😄 Boleh kasih tahu namanya atau sedikit konteks tentang dia biar aku nggak salah paham ✨"
          ...
          if (parsed.isFood) {
            ...
            if (parsed.needsClarification && !userText.trim()) {
              const defaultClarification = isMia
                ? `📸 Fotonya sudah aku cek ya! Biar hitungan nutrisinya akurat, boleh kasih tahu isian utamanya apa? Misalnya sosis, telur, daging, atau lainnya ✨`
