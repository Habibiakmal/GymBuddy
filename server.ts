...
const MIDTRANS_SERVER_KEY = (process.env.MIDTRANS_SERVER_KEY || "SB-Mid-server-YNrlCVGdVDggjHhvMP_yW68w").trim();
const MIDTRANS_CLIENT_KEY = (process.env.VITE_MIDTRANS_CLIENT_KEY || "SB-Mid-client-CAT2gMLueDV0amD7").trim();
const isSandboxKey = MIDTRANS_SERVER_KEY.startsWith("SB-");
const MIDTRANS_IS_PRODUCTION = !isSandboxKey && (process.env.MIDTRANS_IS_PRODUCTION === 'true');
...
// Global concurrency lock to prevent race-condition duplicate account creation
export const activeRegistrationLocks = new Set<string>();

/**
 * Checks whether a phone number already belongs to an existing GymBuddy account.
 * Checks canonical E.164 (+628...) across in-memory cache, dbData.users, demo accounts, and Firestore.
 */
export async function isExistingUserPhone(phoneInput: string): Promise<boolean> {
  const canonical = normalizePhoneToE164(phoneInput);
  if (!canonical) return false;

  // 1. Check in-memory dbData.users direct keys
  ...
  // 2. Check local dbData.users
  if (dbData && dbData.users) {
    for (const [k, u] of Object.entries(dbData.users)) {
      if (!u) continue;
      const uCanonical = (u as any).normalizedPhone || normalizePhoneToE164((u as any).phone || k);
...
export async function handleWhatsAppLoginConfirmation(rawPhone: string, userText: string): Promise<string | null> {
  ...
  for (const sess of authPendingSessions.values()) {
    if (
      (sess.canonicalPhone === canonicalPhone ||
        sess.normPhone === normPhone ||
        sess.altPhone === normPhone ||
        sess.phone === normPhone ||
        sess.phone === altPhone ||
        sess.phone === canonicalPhone) &&
      sess.status === "pending" &&
...
/**
 * Generates an onboarding handshake welcome response for new users connecting on WhatsApp.
 * Welcomes the user warmly, acknowledges their program, provides clear instructions,
 * and asks how they would like to begin.
 * 
 * CRITICAL: MUST NEVER log workouts, meals, or any fake data.
 */
export function generateOnboardingHandshakeResponse(userData: ReturnType<typeof calculateUserData>): string {
  const isMia = (userData.persona || "mia").toLowerCase().includes("mia");
  const coachName = isMia ? "Coach Mia" : "Coach Max";
  const addressing = getValidatedUserAddressing(userData);
  const name = addressing.validatedAddress || userData.name || "Sobat";
  const goalTitle = userData.goalTitle || (
    userData.goal === "lose"
      ? "Menurunkan Berat Badan"
      : userData.goal === "gain"
      ? "Menaikkan Berat Badan"
      : "Program Maintain / Gaya Hidup Sehat"
  );
  ...
  if (isMia) {
    return (
      `Halo ${name}! Senang banget kamu sudah resmi terhubung di GymBuddy 🎉\n` +
      `Aku *Coach Mia*, pelatih nutrisi & kebugaran pribadimu! ✨\n\n` +
      `Program kamu: *${goalTitle}*\n` +
      `🎯 Target Harian: *~${targetCal} kcal* (Protein: *~${targetProtein}g*)\n\n` +
      `Di WhatsApp ini, kamu bisa langsung:\n` +
      `📸 *Kirim foto makanan*: Aku akan analisis nutrisi, porsi & kalorinya otomatis.\n` +
      `🏋️‍♂️ *Menu & Catat Latihan*: Tanyakan menu latihan hari ini, atau laporkan latihan yang selesai kamu lakukan (contoh: _"Saya baru selesai gym 45 menit"_).\n` +
      `⚖️ *Update Berat Badan*: Ketik update berat badan kapan saja (contoh: _"Berat saya sekarang 72 kg"_).\n` +
      `💬 *Tanya Jawab*: Konsultasi seputar nutrisi, diet, atau tips latihan kapan saja!\n\n` +
      `Mau mulai dari mana hari ini, ${name}? Mau cek rekomendasi menu makan, jadwal latihan, atau ada yang ingin ditanyakan seputar programmu? 😊`
    );
  } else {
    return (
      `Halo ${name}! Selamat bergabung di GymBuddy bro! 💪\n` +
      ...
      `📸 *Foto Makanan*: Langsung kirim foto apa yang lo makan buat gue hitung kalorinya.\n` +
      `🏋️‍♂️ *Latihan*: Minta jadwal workout hari ini atau lapor sesi yang beres lo lakuin (contoh: _"Saya baru selesai gym 45 menit"_).\n` +
      `⚖️ *Lapor BB*: Update timbangan lo kapan saja (contoh: _"Berat saya sekarang 72 kg"_).\n` +
      `💬 *Konsultasi*: Tanya teknik gerakan, suplemen, atau strategi program lo!\n\n` +
      `Lo mau mulai dari mana sekarang, ${name}? Mau lihat jadwal latihan hari ini atau cek target nutrisi lo? 🔥`
    );
  }
}

/**
 * Answers questions about the user's program/target without creating workout or meal logs.
 */
export function generateProgramQuestionResponse(userData: ReturnType<typeof calculateUserData>, userText: string): string {
  const isMia = (userData.persona || "mia").toLowerCase().includes("mia");
  const addressing = getValidatedUserAddressing(userData);
  const name = addressing.validatedAddress || userData.name || "Sobat";
  ...
  if (isMia) {
    return (
      `Hai ${name}! Ini gambaran ringkas program kamu di GymBuddy ✨\n\n` +
      `📋 *Program*: ${goalTitle}\n` +
      `🔥 *Target Kalori*: ~${targetCal} kcal / hari\n` +
      `🥩 *Target Protein*: ~${targetProtein}g\n` +
      `🍚 *Target Karbo*: ~${targetCarbs}g\n` +
      `🥑 *Target Lemak*: ~${targetFat}g\n\n` +
      ...
      `Ketik *\"jadwal latihan hari ini\"* atau *\"rekomendasi makan\"* kalau mau langsung lanjut! 😊`
    );
  } else {
    return (
      `Yo ${name}! Ini ringkasan program lo di GymBuddy: 🔥\n\n` +
      `📋 *Program*: ${goalTitle}\n` +
      `🔥 *Target Kalori*: ~${targetCal} kcal / hari\n` +
      `🥩 *Target Protein*: ~${targetProtein}g\n` +
      `🍚 *Target Karbo*: ~${targetCarbs}g\n` +
      ...
      `Ketik *\"jadwal workout hari ini\"* atau *\"saran makan\"* buat gas langsung bro! 💪`
    );
  }
}

/**
 * Handles vague workout mentions (e.g. "Saya baru latihan") by asking for clarification
 * instead of fabricating exercises.
 */
export function generateVagueWorkoutClarificationResponse(userData: ReturnType<typeof calculateUserData>, userText: string): string {
  const isMia = (userData.persona || "mia").toLowerCase().includes("mia");
  const addressing = getValidatedUserAddressing(userData);
  const name = addressing.validatedAddress || userData.name || "Sobat";
  ...
  if (isMia) {
    return (
      `Hebat sudah aktif berolahraga hari ini, ${name}! 🎉\n\n` +
      ...
      `Supaya bisa aku catat lengkap di Dashboard kamu ya! 💪`
    );
  } else {
    return (
      `Mantap udah kelar latihan bro, ${name}! 🔥\n\n` +
      ...
      `Ketik aja di sini, langsung gue rekap! 💪`
    );
  }
}
...
export interface PendingWorkoutClarification {
  phone: string;
  durationMinutes?: number;
...
export const pendingWorkoutClarifications = new Map<string, PendingWorkoutClarification>();
...
const ADDITIONAL_ACTIVITY_MAP = [
  ...
  { keywords: ["gym", "fitness", "fitnes", "angkat beban", "latihan beban", "weight training", "strength training"], name: "Latihan Beban (Gym)", icon: "🏋️‍♂️", category: "strength", met: 6 }
];
...
export function handleAdditionalActivityLogging(
  ...
  // Guard 0: Strict Intent Classification Gate
  // Conversational introductions, onboarding greetings, and program queries must NEVER log workout
  const classifiedIntent = classifyUserIntent(userText);
  if (
    classifiedIntent.intent === "ONBOARDING_GREETING" ||
    classifiedIntent.intent === "GENERAL_CONVERSATION" ||
    classifiedIntent.intent === "PROGRAM_QUESTION" ||
    classifiedIntent.intent === "WORKOUT_QUESTION" ||
    classifiedIntent.intent === "NUTRITION_QUESTION" ||
    classifiedIntent.intent === "WEIGHT_LOG" ||
    classifiedIntent.intent === "VAGUE_WORKOUT_NEEDS_CLARIFICATION"
  ) {
    return null;
  }
  ...
  // Sanitize text: strip "gymbuddy" so brand name does not falsely trigger "gym"
  const sanitized = sanitizeTextForIntent(lower);

  // Match activity type using word boundary regex on sanitized text
  let matchedAct = null;
  for (const act of ADDITIONAL_ACTIVITY_MAP) {
    ...
    if (hasWordMatch) {
      // If it matches gym/strength but user provided specific sets/reps/weight, let strength logging handle it
  ...
  // STRICT SAFETY GUARD:
  // Activity logging MUST have either an explicit completion signal OR specific duration/distance/sets
  // NEVER invent or assume a workout session just because a keyword was mentioned!
  if (!hasCompletionSignal && !params.durationMinutes && !params.durationSeconds && !params.distanceKm && !isStrengthStructured) {
    return null;
  }

  // Extract duration, distance, intensity (NEVER invent default duration)
  ...
  // Calorie Burn Estimation (MET * Weight * Hours) - strictly undefined if duration/distance unknown
  const weight = userData.weight || 70;
  const durHours = duration ? duration / 60 : (distance ? (distance / 10) : (params.durationSeconds ? params.durationSeconds / 3600 : undefined));
  const calBurn = durHours !== undefined ? Math.max(5, Math.round(matchedAct.met * weight * durHours)) : undefined;
  ...
  if (isEdit && existingActivities.length > 0) {
    ...
    if (existingIndex !== -1) {
      ...
      if (calBurn !== undefined) existingActivities[existingIndex].estimatedCaloriesBurned = calBurn;

      dbData.dailyLogs[actKey] = existingActivities;
      dbData.dailyLogs[altActKey] = existingActivities;
      saveDb();

      const editCalStr = calBurn !== undefined ? ` (~${calBurn} kcal estimasi)` : "";
      return [
  ...
  if (calBurn !== undefined) details.push(`🔥 Estimasi Bakar: *~${calBurn} kcal*`);
...
export function handleWorkoutProgressLogging(
  ...
  // Guard -1: Strict Intent Classification Gate
  // Conversational greetings, onboarding handshakes, and program queries must NEVER log workout
  const classifiedIntent = classifyUserIntent(userText);
  if (
    classifiedIntent.intent === "ONBOARDING_GREETING" ||
    classifiedIntent.intent === "GENERAL_CONVERSATION" ||
  ...
  // Check 0: Active Pending Clarification
  // If user previously said e.g. "ku sudah olahraga 45 menit" and we asked what activity, check if user now provides the activity!
  const pending = pendingWorkoutClarifications.get(phone) || pendingWorkoutClarifications.get(altPhone);
  const isPendingActive = Boolean(pending && (Date.now() - pending.timestamp < 30 * 60 * 1000));
  ...
  if (isPendingActive && pending) {
    ...
    if (matchedPendingAct) {
      // User specified the activity in response to the clarification question!
      ...
      const weight = userData.weight || 70;
      const durHours = duration ? duration / 60 : (distance ? (distance / 10) : (durSeconds ? durSeconds / 3600 : undefined));
      const calBurn = durHours !== undefined ? Math.max(5, Math.round(matchedPendingAct.met * weight * durHours)) : undefined;
      ...
      let pExistingActivities: AdditionalActivity[] = dbData.dailyLogs[pActKey] || dbData.dailyLogs[pAltActKey] || [];
      if (!Array.isArray(pExistingActivities)) pExistingActivities = [];

      const actId = `act-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
      const newAct: AdditionalActivity = {
        id: actId,
      ...
      const coachCommentText = userData.persona === "max"
        ? `Bagus, ${validatedAddr}! Aktivitas ${matchedPendingAct.name} sudah tercatat rapi. Tetap jaga hidrasi & makan bergizi! 🔥`
        : `Bagus banget, ${validatedAddr}! Aktivitas ${matchedPendingAct.name} sudah tercatat. Tetap aktif bergerak dan jangan lupa istirahat ya ✨`;
      const comment = validateAndFormatCoachNote(coachCommentText, userData);
  ...
  // Check full workout completion (e.g. "selesai semua jadwal latihan hari ini", "jadwal workout hari ini sudah selesai semua")
  // NOTE: Must NEVER match generic completion like "ku sudah olahraga 45 menit"
  const isFullWorkout = Boolean(lower.match(/(?:selesai|beres|done)\s+semua\s+(?:jadwal|menu|program|latihan|workout)(?:\s+hari\s*ini)?/i)) ||
    Boolean(lower.match(/^(?:semua\s+)?(?:jadwal|menu|program)\s+(?:workout|latihan)?\s*(?:hari\s*ini\s*)?(?:sudah\s*)?(?:selesai|beres|done)$/i));

  // Check generic workout intent without specific activity (e.g. "ku sudah olahraga 45 menit", "aku sudah olahraga", "tadi workout 30 menit")
  ...
  if (isGenericWorkoutSignal && !dbEx && !hasStructure && !isFullWorkout) {
    // 1. Detect workout logging intent correctly
    // 2. Extract duration / parameters
    // 3. Activity is UNKNOWN -> DO NOT calculate or invent calories burned
    // 4. Do NOT assume the user performed today's scheduled workout
    // 5. Ask a concise clarification and save pending state
    ...
    if (durText) {
      clarificationMsg = userData.persona === "max"
        ? `Siap, durasi ${durText}-nya udah gue catat. Lo tadi olahraga apa? Misalnya jalan kaki, lari, gym, atau HIIT, biar estimasi kalorinya akurat! 🔥`
        : `Siap, aku bisa bantu catat ${durText}-nya. Kamu tadi olahraga apa? Misalnya jalan kaki, lari, gym, atau HIIT, supaya aku bisa bantu catat dengan lebih tepat ya ✨`;
    } else {
      clarificationMsg = userData.persona === "max"
        ? `Siap! Lo tadi olahraga apa dan berapa lama? Misalnya jalan kaki 30 menit, lari, gym, atau HIIT, biar bisa langsung gue catat! 🔥`
        : `Siap, aku bisa bantu catat olahragamu! Kamu tadi olahraga apa dan berapa lama durasinya? Misalnya jalan kaki 30 menit, lari, gym, atau HIIT, supaya aku bisa bantu catat dengan lebih tepat ya ✨`;
    }
...
export async function createExpressApp(options: { skipVite?: boolean } = {}) {
  ...
  // ─── Authoritative Phone Identity Gate (Phone Before Plan Selection & Payment) ──────────
  app.post("/api/auth/check-phone", express.json(), async (req, res) => {
    try {
      ...
      // Check if canonical phone exists in system
      const exists = await isExistingUserPhone(canonicalPhone);
      if (exists) {
        const user = (await findUserByPhoneOrId(canonicalPhone)) || 
                     (normPhone ? await findUserByPhoneOrId(normPhone) : null) || 
                     getUserProfile(canonicalPhone) || 
                     (normPhone ? getUserProfile(normPhone) : null) ||
                     (dbData.users && (dbData.users[canonicalPhone] || (normPhone && dbData.users[normPhone])));
        ...
        let nextStep: "login" | "onboarding" | "plan_selection" = "login";
  ...
  app.post("/api/auth/login-request", express.json(), async (req, res) => {
    try {
      ...
      let user = (await findUserByPhoneOrId(canonicalPhone)) ||
                 ...
                 getUserProfile(normPhone) ||
                 (await findUserByPhoneOrId(altPhone)) ||
                 getUserProfile(altPhone);
  ...
  app.post("/api/auth/login-resend", express.json(), async (req, res) => {
    ...
    const waMsg = [
      `🔐 *Kode Verifikasi GymBuddy (Kirim Ulang)*`,
      ``,
  ...
  app.post("/api/onboarding", async (req, res) => {
    ...
    // Concurrency Lock to prevent race-condition duplicate accounts
    ...
    try {
      // Backend Uniqueness Check: Never overwrite existing user during onboarding!
      ...
      if (alreadyExists) {
        ...
        if (existingProfile && (existingProfile as any).passwordHash) {
          return res.status(409).json({
            success: false,
            error: "account_already_exists",
  ...
  // Save pre-payment onboarding profile (does not require phone number)
  app.post("/api/onboarding/profile", express.json(), async (req, res) => {
    try {
  ...
  // ─── Canonical Order Creation & Purchase Classification Logic ───────────────────────
  async function handleCreateOrderLogic(reqBody: any, authHeader?: string): Promise<{ status: number; body: any }> {
    const {
      userId,
      phone,
      plan = "free",
    ...
    const existingUser =
      (canonicalPhone ? await findUserByPhoneOrId(canonicalPhone) : null) ||
      (localPhone ? await findUserByPhoneOrId(localPhone) : null) ||
      (effectiveUserId ? await findUserByPhoneOrId(effectiveUserId) : null) ||
      (canonicalPhone ? getUserProfile(canonicalPhone) : null) ||
      (localPhone ? getUserProfile(localPhone) : null) ||
      (effectiveUserId && dbData.users ? dbData.users[effectiveUserId] : null);

    const pendingProfile =
      (effectiveUserId && dbData.pendingProfiles ? dbData.pendingProfiles[effectiveUserId] : null) ||
      existingUser ||
      {};

    // Validate plan & determine purchase type (new_purchase, upgrade, renewal, invalid_duplicate)
    ...
    if (normalizedPlan === "trial" || String(plan).toLowerCase() === "free" || String(plan).toLowerCase() === "free_trial") {
      ...
      const orderId = `GB-ORD-FREE-${(localPhone || effectiveUserId.replace(/[^a-zA-Z0-9]/g, "")).substring(0, 14)}-${Date.now()}`;
      const order = {
        orderId,
        ...
        plan: "trial",
        planType: "free",
        purchaseType: "new_purchase" as PurchaseType,
        activeService: "both",
        feature: "both",
        price: 0,
        amount: 0,
      ...
      return {
        status: 200,
        body: { success: true, orderId, order, purchaseType: "new_purchase" }
      };
    }
    ...
    const canonicalPlanKey: PlanKey = normalizedPlan === "both" ? "both" : (normalizedPlan === "workout_coach" ? "workout_coach" : "nutritionist");
    const durationMonths = PLAN_PRICING[canonicalPlanKey]?.[canonicalDuration]?.durationMonths || 3;

    // Authoritative price calculation from pricing config
    ...
    // Deduplication: Check if an unexpired pending order exists for this user and configuration (<24 hours)
    ...
    for (const [existingId, ord] of Object.entries(dbData.orders)) {
      if (
        ...
        ord.status === "pending" &&
        ord.midtransToken &&
        (ord.userId === effectiveUserId || (canonicalPhone && ord.whatsappNumber === canonicalPhone) || (localPhone && ord.whatsappNumber === localPhone)) &&
        ord.selectedPlan === normalizedPlan &&
        ord.billingPeriod === canonicalDuration &&
        ord.amount === grossAmount
      ) {
  ...
  // Retry order payment
  app.post("/api/orders/:orderId/retry", express.json(), async (req, res) => {
    try {
      const { orderId } = req.params;
      const order = dbData.orders ? dbData.orders[orderId] : null;
      if (!order) {
      ...
      order.lastRetryTimestamp = new Date().toISOString();
      dbData.orders[orderId] = order;
  ...
  // Connect WhatsApp number to profile and order
  app.post("/api/account/connect-whatsapp", express.json(), async (req, res) => {
    ...
    activeRegistrationLocks.add(canonicalPhone);
    try {
      let baseProfile = (dbData.pendingProfiles && dbData.pendingProfiles[userId]) ||
                        (dbData.users && dbData.users[userId]) ||
                        req.body.profileFallback ||
                        null;
      ...
      if (order && order.paymentStatus === "paid") {
        ...
        const canonicalDuration = order.billingPeriod === "lifetime" ? "lifetime" : 
                                 ...
                                 order.billingPeriod === "3m" ? "3_months" : "1_month";
...
Keluarkan HANYA JSON valid tanpa teks markdown di luar JSON:
{
  ...
  app.post("/api/user/:phone/meals", express.json(), requireAuthMiddleware, requireOwnershipMiddleware, requireEntitlementMiddleware("nutrition"), async (req, res) => {
    ...
    const upsertMealInList = (list: MealLog[], item: MealLog) => {
  ...
  // Midtrans Payment Endpoint (Delegates to canonical order creation logic)
  ...
  app.post("/api/midtrans/notification", express.json(), express.urlencoded({ extended: true }), async (req, res) => {
    try {
      ...
      // Extract phone, plan, and duration from custom fields or order metadata
      ...
      // Link to order in dbData.orders if present
      ...
      let canonicalDuration: CanonicalPlanDuration = "1_month";
      let daysToAdd = 30;
      ...
      let canonicalPlan: CanonicalPlan = "premium";
      if (plan === "lifetime") {
      ...
      if (isSuccess && phone) {
        ...
        // Resolve user across all memory, cache, and DB stores
        const canonicalPhoneKey = normalizePhoneToE164(phone);
        const existingUser = (canonicalPhoneKey ? await findUserByPhoneOrId(canonicalPhoneKey) : null) ||
                             (await findUserByPhoneOrId(normPhone)) ||
                             dbData.users[normPhone] ||
                             (canonicalPhoneKey ? dbData.users[canonicalPhoneKey] : null) ||
                             getUserProfile(normPhone) ||
                             (canonicalPhoneKey ? getUserProfile(canonicalPhoneKey) : null) || {
          phone: normPhone,
        ...
        const purchaseType: PurchaseType = ord?.purchaseType || (
          rawServiceField.includes(":") && rawServiceField.split(":")[2]
            ? (rawServiceField.split(":")[2] as PurchaseType)
            : (finalPlan === "premium" && existingSubPlan && existingSubPlan !== "premium" ? "upgrade" : "new_purchase")
        );

        const applied = applyCommercialPlan(existingUser, finalPlan, canonicalDuration, purchaseType);
        const resolvedExpiresAt = applied.success && applied.user?.planExpiresAt
          ? new Date(applied.user.planExpiresAt)
          : (canonicalDuration === "lifetime" ? null : new Date(Date.now() + daysToAdd * 24 * 3600 * 1000));
        ...
        try {
          ...
          const durationLabel = canonicalDuration === "lifetime" ? "LIFETIME (Akses Selamanya)" :
            canonicalDuration === "1_year" ? "1 Tahun" :
            canonicalDuration === "6_months" ? "6 Bulan" :
            canonicalDuration === "3_months" ? "3 Bulan" : "1 Bulan";
          const confirmMsg =
            `🎉 *PEMBAYARAN BERHASIL!*\n` +
            `--------------------------------------------------\n` +
