...
function getOrCreateUserProfile(rawPhone) {
  ...
  if (!user) {
    ...
    const validUsers = Object.entries(dbData.users).filter(([key]) => key !== "latest_onboarding").map(([, u]) => u);
    const matchedByPhone = validUsers.find((u) => normalizePhone(u.phone || "") === phone);
    if (matchedByPhone) {
...
function addMealLog(rawPhone, meal, targetDateStr) {
  ...
  for (const itemMeal of mealsToInsert) {
    ...
    for (const uPhone of Object.keys(dbData.users)) {
      ...
      if (!dbData.dailyLogs[uKey].some((m) => m.id === itemMeal.id)) {
